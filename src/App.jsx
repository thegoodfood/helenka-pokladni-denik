...
function VkladForm({ user, store, firmy, defF, onBack, nt, typ }) {
  ...
  const doSave = async () => {
    ...
    try {
      ...
      try { await try { await store.exportToSheets(saved || txData, user.jmeno, Number(firma), firmaNazev, null); } catch(se) { console.warn("Sheets:", se.message); } } catch(se) { console.warn("Sheets:", se.message); }
...
function TxForm({ user, store, firmy, defF, onBack, nt }) {
  ...
  const doSave = async () => {
    ...
    try {
      ...
      try { await try { await store.exportToSheets(saved || txData, user.jmeno, Number(f.firma_id), firmaNazev, katName); } catch(se) { console.warn("Sheets:", se.message); } } catch(se) { console.warn("Sheets:", se.message); }
      if (fileName) store.uploadToDrive(fileName, user.jmeno, firmaNazev);
      setShowSuccess(true);
    } catch (e) {
      nt("Chyba při ukládání: " + e.message, "error");
